...
| Fully frozen fragments | **33 / 96** | page-resolved conversion phase active |
...
The union of all `fragment_locator_progress*.csv` shards contains **83/96** pilot slots. Thirty are frozen. The remaining **50** located rows include exact scholarly page pointers, section starts, reproduced facsimiles and snippet-resolved candidates that have not yet crossed the exact-boundary primary-inspection gate. **13 slots remain without a locator.**
